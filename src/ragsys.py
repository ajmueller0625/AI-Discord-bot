...
class RAGSystem:
    def __init__(self, db_session: Session, embedding_model: str, threshold: float):
        ...
        self.logger.info(
            f'Threshold similarity is set to: {self.similarity_threshold}')
    ...
    def generate_embedding(self, text: str) -> list[float]:
        ...
        try:
            ...
            self.logger.info(
                f'Embedding generated successfully with dimension: {len(result)}')
            return result
    ...
    def find_similar_question(self, question: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f'Database error during similarity search: {str(e)}')
                self.logger.error(traceback.format_exc())
                raise
            ...
            if result and result[2] >= self.similarity_threshold:
                self.logger.info(
                    f'Found similar question with id: {result[0]} and similarity: {result[2]}')
                return {'id': result[0], 'similarity': result[2]}
            else:
                self.logger.info(
                    'No similar question found or similarity below threshold')
                return None
    ...
    def get_answer(self, question_id: int) -> str:
        ...
        try:
            ...
            try:
                ...
                self.logger.error(
                    f'Database error during answer retrieval: {str(e)}')
                self.logger.error(traceback.format_exc())
                raise
            ...
            if result:
                self.logger.info(
                    f'Answer retrieved successfully, length: {len(result[0].answer)}')
                return result[0].answer
            else:
                self.logger.info(
                    'No answer found for question id: {question_id}')
                return None
    ...
    def save_question_answer(self, question: str, answer: str, is_verified: bool = True) -> tuple[Question, Answer]:
        ...
        try:
            ...
            new_question = Question(
                question=question, question_embeddings=question_embedding)

            # Create a new answer
            new_answer = Answer(question=new_question,
                                answer=answer, is_verified=is_verified)
            ...
            try:
                ...
                self.logger.info(
                    f'Question and answer saved successfully with question id: {new_question.id}')
                return new_question, new_answer
            except SQLAlchemyError as e:
                self.db_session.rollback()
                self.logger.error(
                    f'Database error during saving question and answer: {str(e)}')
                self.logger.error(traceback.format_exc())
                return None, None
